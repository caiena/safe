...
describe SAFE::Workflow do
  ...
  describe "#run" do
    it "allows passing additional params to the job" do
      flow = SAFE::Workflow.new
      flow.run(SAFE::Job, params: { something: 1 })
      flow.save
    ...
    context "when graph is empty" do
      it "adds new job with the given class as a node" do
        ...
        flow.run(SAFE::Job)
        flow.save
        expect(flow.jobs.first).to be_instance_of(SAFE::Job)
      end
    end

    it "allows `after` to accept an array of jobs" do
      tree = SAFE::Workflow.new
      klass1 = Class.new(SAFE::Job)
      klass2 = Class.new(SAFE::Job)
      klass3 = Class.new(SAFE::Job)
